...
class PINN_laminar_flow:
    ...
    def __init__(self, Collo, INLET, internal, OUTLET_d, OUTLET_n, CYLD, TOP_BOTTOM, uv_layers, lb, ub, ExistModel, uvDir):
        ...
        _, _, self.p_OUTLET_pred2, _, _, _ = self.net_uv(self.x_OUTLET_n_tf, self.y_OUTLET_n_tf)        
        ...
        self.loss_OUTLET = tf.reduce_mean(tf.square(self.p_OUTLET_pred2-self.p_OUTLET_n_tf))
        ...
        if use_internal == 0:
            if use_neural_form==0:
                ...
                self.loss_bc = (self.cyld_weight*self.loss_CYLD + self.wall_weight*self.loss_TOP_BOTTOM + self.inlet_weight*self.loss_INLET + self.outlet_weight*self.loss_OUTLET)
                self.loss = self.loss_f + (self.cyld_weight*self.loss_CYLD + self.wall_weight*self.loss_TOP_BOTTOM + self.inlet_weight*self.loss_INLET + self.outlet_weight*self.loss_OUTLET)
    ...
    def train(self, iter, learning_rate, initial):
        ...
        for it in range(iter):
            ...
            loss_OUTLET_stress.append(self.sess.run(self.loss_stress_free,tf_dict))
        ...
        loss_OUTLET_dirichlet,loss_OUTLET_neumann, loss_f, self.loss, iterate_count, loss_list, loss_TOP_BOTTOM, loss_INLET, loss_OUTLET_stress, loss_OUTLET
...
if __name__ == "__main__":
    ...
    uv_layers = [2] + 16*[40] + [6]  # 2 i/p, 8 layers x 40 neurons each, 6 o/p
    ...
    x_CYLD_1 = np.multiply(r, np.cos(theta))+0.0
    y_CYLD_1 = np.multiply(r, np.sin(theta))+0.0
    u_CYLD_1 = np.zeros_like(x_CYLD_1)
    v_CYLD_1 = np.zeros_like(x_CYLD_1)
    u_CYLD_1[:] = 0.
    v_CYLD_1[:] = 0.

    theta = [0.0] + [2*np.pi] * lhs(1, cylinder_pts)
    x_CYLD_2 = np.multiply(r, np.cos(theta))+ 3
    y_CYLD_2 = np.multiply(r, np.sin(theta))+0.0
    u_CYLD_2 = np.zeros_like(x_CYLD_2)
    v_CYLD_2 = np.zeros_like(x_CYLD_2)
    u_CYLD_2[:] = 0.
    v_CYLD_2[:] = 0.

    x_CYLD = np.concatenate((x_CYLD_1, x_CYLD_2), 0)
    y_CYLD = np.concatenate((y_CYLD_1, y_CYLD_2), 0)
    u_CYLD = np.concatenate((u_CYLD_1, u_CYLD_2), 0)
    v_CYLD = np.concatenate((v_CYLD_1, v_CYLD_2), 0)
    ...
    if CFD_grid ==0:
        ...
        XY_c_refine_upper = [-1.0, 0] + [11.0, 2.0] * lhs(2, int(refine_pts/2))  #upper symmetrical for refined domain
        ...
        XY_c_refine=DelCylPT(XY_c_refine, xc=3.0, yc= 0.0,r=0.5)
        XY_c = np.concatenate((XY_c, XY_c_refine), 0)
        XY_c = DelCylPT(XY_c, xc=0.0, yc=0.0, r=0.5)   #remove pts inside circle
        XY_c = DelCylPT(XY_c, xc=3.0, yc=0.0, r=0.5)   #remove pts inside circle
    ...
    with tf.device('/device:GPU:0'):
        ...
        if debug == 0:
            if use_old_model==1:
                ...
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, loss_TOP_BOTTOM, loss_INLET, loss_OUTLET_stress, loss_OUTLET = model.train(iter=2000, learning_rate=0.0005, initial=2)
            else:
                loss_CYLD, loss_CYLD_neumann, loss_internal, loss_OUTLET_dirichlet, \
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, loss_TOP_BOTTOM, loss_INLET, loss_OUTLET_stress, loss_OUTLET = model.train(iter=10000, learning_rate=0.0005, initial=2)
        
        elif debug == 1:
            
                loss_CYLD, loss_CYLD_neumann, loss_internal, loss_OUTLET_dirichlet, \
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, loss_TOP_BOTTOM, loss_INLET, loss_OUTLET_stress, loss_OUTLET= model.train(iter=60, learning_rate=0.0005, initial=1)
        ...
        if CFD_grid ==0:
            ...
            dst2 = ((x_PINN-3.0)**2+(y_PINN-0.0)**2)**0.5
            x_PINN = x_PINN[dst2 >= 0.5]
            y_PINN = y_PINN[dst2 >= 0.5]
            x_PINN = x_PINN.flatten()[:, None]
            y_PINN = y_PINN.flatten()[:, None]
    ...
    fx_p,fy_p = pressure_forces2(surface_pts)
