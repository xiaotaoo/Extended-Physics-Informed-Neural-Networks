...
class XPINN_laminar_flow:
    ...
    def __init__(self, collo1, collo2, interface, INLET, OUTLET, internal1, internal2, CYLD1, CYLD2, WALL1, WALL2, uv_layers, lb, ub, ExistModel, uvDir):
        ...
        #Cylinder pts
        self.x_CYLD1 = CYLD1[:, 0:1]
        self.y_CYLD1 = CYLD1[:, 1:2]
        self.u_CYLD1 = CYLD1[:, 2:3]
        self.v_CYLD1 = CYLD1[:, 3:4]

        self.x_CYLD2 = CYLD2[:, 0:1]
        self.y_CYLD2 = CYLD2[:, 1:2]
        self.u_CYLD2 = CYLD2[:, 2:3]
        self.v_CYLD2 = CYLD2[:, 3:4]
        #internal pts
        self.x_internal1 = internal1[:, 0:1]
        self.y_internal1 = internal1[:, 1:2]
        self.u_internal1 = internal1[:, 2:3]
        self.v_internal1 = internal1[:, 3:4]
        self.p_internal1 = internal1[:, 4:5]

        self.x_internal2 = internal2[:, 0:1]
        self.y_internal2 = internal2[:, 1:2]
        self.u_internal2 = internal2[:, 2:3]
        self.v_internal2 = internal2[:, 3:4]
        self.p_internal2 = internal2[:, 4:5]

        self.x_OUTLET = OUTLET[:, 0:1]
        self.y_OUTLET = OUTLET[:, 1:2]
        self.p_OUTLET = OUTLET[:, 2:3]

        self.x_WALL1 = WALL1[:, 0:1]
        self.y_WALL1 = WALL1[:, 1:2]
        self.u_WALL1 = WALL1[:, 2:3]
        self.v_WALL1 = WALL1[:, 3:4]

        self.x_WALL2 = WALL2[:, 0:1]
        self.y_WALL2 = WALL2[:, 1:2]
        self.u_WALL2 = WALL2[:, 2:3]
        self.v_WALL2 = WALL2[:, 3:4]
        ...
        if ExistModel== 0 :
            ...
            print("Loading uv NN ...")
            self.weights1, self.biases1, self.weights2, self.biases2 = self.load_NN(uvDir, uv_layers)

        # tf placeholders
        self.learning_rate = tf.placeholder(tf.float32, shape=[])
        self.inlet_weight1 = tf.placeholder(tf.float32, shape=[])
        self.inlet_weight2 = tf.placeholder(tf.float32, shape=[])
        self.outlet_weight1 = tf.placeholder(tf.float32, shape=[])      
        self.outlet_weight2 = tf.placeholder(tf.float32, shape=[])  
        ...
        self.x_WALL1_tf = tf.placeholder(tf.float32, shape=[None, self.x_WALL1.shape[1]])
        self.y_WALL1_tf = tf.placeholder(tf.float32, shape=[None, self.y_WALL1.shape[1]])
        self.u_WALL1_tf = tf.placeholder(tf.float32, shape=[None, self.u_WALL1.shape[1]])
        self.v_WALL1_tf = tf.placeholder(tf.float32, shape=[None, self.v_WALL1.shape[1]])
        
        self.x_WALL2_tf = tf.placeholder(tf.float32, shape=[None, self.x_WALL2.shape[1]])
        self.y_WALL2_tf = tf.placeholder(tf.float32, shape=[None, self.y_WALL2.shape[1]])
        self.u_WALL2_tf = tf.placeholder(tf.float32, shape=[None, self.u_WALL2.shape[1]])
        self.v_WALL2_tf = tf.placeholder(tf.float32, shape=[None, self.v_WALL2.shape[1]])

        self.x_CYLD1_tf = tf.placeholder(tf.float32, shape=[None, self.x_CYLD1.shape[1]])
        self.y_CYLD1_tf = tf.placeholder(tf.float32, shape=[None, self.y_CYLD1.shape[1]])
        self.u_CYLD1_tf = tf.placeholder(tf.float32, shape=[None, self.u_CYLD1.shape[1]])
        self.v_CYLD1_tf = tf.placeholder(tf.float32, shape=[None, self.v_CYLD1.shape[1]])

        self.x_CYLD2_tf = tf.placeholder(tf.float32, shape=[None, self.x_CYLD2.shape[1]])
        self.y_CYLD2_tf = tf.placeholder(tf.float32, shape=[None, self.y_CYLD2.shape[1]])
        self.u_CYLD2_tf = tf.placeholder(tf.float32, shape=[None, self.u_CYLD2.shape[1]])
        self.v_CYLD2_tf = tf.placeholder(tf.float32, shape=[None, self.v_CYLD2.shape[1]])
        ...
        self.x_internal1_tf = tf.placeholder(tf.float32, shape=[None, self.x_internal1.shape[1]])
        self.y_internal1_tf = tf.placeholder(tf.float32, shape=[None, self.y_internal1.shape[1]])
        self.u_internal1_tf = tf.placeholder(tf.float32, shape=[None, self.u_internal1.shape[1]])
        self.v_internal1_tf = tf.placeholder(tf.float32, shape=[None, self.v_internal1.shape[1]])
        self.p_internal1_tf = tf.placeholder(tf.float32, shape=[None, self.p_internal1.shape[1]])

        self.x_internal2_tf = tf.placeholder(tf.float32, shape=[None, self.x_internal2.shape[1]])
        self.y_internal2_tf = tf.placeholder(tf.float32, shape=[None, self.y_internal2.shape[1]])
        self.u_internal2_tf = tf.placeholder(tf.float32, shape=[None, self.u_internal2.shape[1]])
        self.v_internal2_tf = tf.placeholder(tf.float32, shape=[None, self.v_internal2.shape[1]])
        self.p_internal2_tf = tf.placeholder(tf.float32, shape=[None, self.p_internal2.shape[1]])
        ...
        # Obtain the predictions by loading the points
        self.u1_pred, self.v1_pred, self.p1_pred, _, _, _ = self.net_u1(self.x1_tf, self.y1_tf)
        self.u2_pred, self.v2_pred, self.p2_pred, _, _, self.sxy2_pred = self.net_u2(self.x2_tf, self.y2_tf)
        ...
        self.u_WALL1_pred, self.v_WALL1_pred, _, _, _, _ = self.net_u1(self.x_WALL1_tf, self.y_WALL1_tf) #For top and bottom in domain 1
        self.u_WALL2_pred, self.v_WALL2_pred, _, _, _, _ = self.net_u2(self.x_WALL2_tf, self.y_WALL2_tf) #For top and bottom in domain 2
        self.u_CYLD1_pred, self.v_CYLD1_pred, _, _, _, _ = self.net_u1(self.x_CYLD1_tf, self.y_CYLD1_tf) #Cylinder in domain 1
        self.u_CYLD2_pred, self.v_CYLD2_pred, _, _, _, _ = self.net_u2(self.x_CYLD2_tf, self.y_CYLD2_tf) #Cylinder in domain 2
        self.u_INLET_pred, self.v_INLET_pred, _, _, _, _ = self.net_u1(self.x_INLET_tf, self.y_INLET_tf) #For inlet in subdomain 1
        self.u_internal1_pred, self.v_internal1_pred, self.p_internal1_pred, _, _, _ = self.net_u1(self.x_internal1_tf, self.y_internal1_tf) 
        self.u_internal2_pred, self.v_internal2_pred, self.p_internal2_pred, _, _, _ = self.net_u2(self.x_internal2_tf, self.y_internal2_tf)      
        
        self.p_bc1, self.p_bc2=self.stressfree_bc(self.x_OUTLET_tf, self.y_OUTLET_tf)


        _, _, self.p_OUTLET_pred, self.s11_OUTLET_pred, _, self.s12_OUTLET_pred = self.net_u2(self.x_OUTLET_tf, self.y_OUTLET_tf) #Outlet domain 2
        ...
        [self.u_x_CYLD_pred1,self.v_x_CYLD_pred1,self.u_y_CYLD_pred1,self.v_y_CYLD_pred1] = \
        Gradient_Velocity_2D(self.u_CYLD1_pred, self.v_CYLD1_pred,self.x_CYLD1_tf, self.y_CYLD1_tf)

        [self.u_x_CYLD_pred2,self.v_x_CYLD_pred2,self.u_y_CYLD_pred2,self.v_y_CYLD_pred2] = \
        Gradient_Velocity_2D(self.u_CYLD2_pred, self.v_CYLD2_pred,self.x_CYLD2_tf, self.y_CYLD2_tf)
    
                
        self.loss_WALL1 = tf.reduce_mean(tf.square(self.u_WALL1_pred-self.u_WALL1_tf)) \
                       + tf.reduce_mean(tf.square(self.v_WALL1_pred-self.v_WALL1_tf))
        self.loss_WALL2 = tf.reduce_mean(tf.square(self.u_WALL2_pred-self.u_WALL2_tf)) \
                       + tf.reduce_mean(tf.square(self.v_WALL2_pred-self.v_WALL2_tf))

        #self.loss_WALL = tf.reduce_mean(tf.square(self.wall_bc))

        self.loss_CYLD1 = tf.reduce_mean(tf.square(self.u_CYLD1_pred-self.u_CYLD1_tf)) \
                       + tf.reduce_mean(tf.square(self.v_CYLD1_pred-self.v_CYLD1_tf))     

        self.loss_CYLD2 = tf.reduce_mean(tf.square(self.u_CYLD2_pred-self.u_CYLD2_tf)) \
                       + tf.reduce_mean(tf.square(self.v_CYLD2_pred-self.v_CYLD2_tf))     
        ...
        self.loss_internal1 = tf.reduce_mean(tf.square(self.u_internal1_pred-self.u_internal1_tf)) \
                         + tf.reduce_mean(tf.square(self.v_internal1_pred-self.v_internal1_tf)) \
                             + tf.reduce_mean(tf.square(self.p_internal1_pred-self.p_internal1_tf))

        self.loss_internal2 = tf.reduce_mean(tf.square(self.u_internal2_pred-self.u_internal2_tf)) \
                         + tf.reduce_mean(tf.square(self.v_internal2_pred-self.v_internal2_tf)) \
                             + tf.reduce_mean(tf.square(self.p_internal2_pred-self.p_internal2_tf))
        ...
        if use_internal == 0:
            ...
            self.loss1 = self.loss_f1 + (self.loss_INLET + self.loss_CYLD1+ self.loss_WALL1) + 8*(self.loss_discontinuous1 + self.loss_continuity)
            self.loss2 = self.loss_f2 + (self.loss_CYLD2 + self.loss_WALL2+ self.loss_OUTLET) + 8*(self.loss_discontinuous2 + self.loss_continuity)
            #self.loss1 = self.loss_f1 + 2*(self.loss_INLET + self.loss_CYLD1+ self.loss_WALL1) + 20*self.loss_discontinuous1 + self.loss_continuity
            #self.loss2 = self.loss_f2 + 2*(self.loss_CYLD2 + self.loss_WALL2+ self.loss_OUTLET) + 20*self.loss_discontinuous2 + self.loss_continuity
            
        else:
            
            self.loss1 = self.loss_f1 + 4*(self.loss_INLET1+ self.loss_OUTLET1 + self.loss_WALL +self.loss_internal1) + self.loss_discontinuous1 + self.loss_continuity
            self.loss2 = self.loss_f2 + (4*self.loss_CYLD + 4*self.loss_INLET2 + 4*self.loss_OUTLET2 + 4*self.loss_internal2) + self.loss_discontinuous2 + self.loss_continuity
        ...
        if debug == 0:
            ...
            self.optimizer1 = tf.contrib.opt.ScipyOptimizerInterface(self.loss1 + 2*self.loss2,
                                                                    var_list=self.weights1 + self.biases1 +self.weights2 +self.biases2,
                                                                    method='L-BFGS-B',
                                                                    options={'maxiter': 100,
                                                                              'maxfun': 90,
        ...
        if optimizer ==0: 
            ...
            self.train_op_Adam1 = self.optimizer_Adam.minimize(loss=self.loss1,
            ...
            self.optimizer_Yellowfin2 = YFOptimizer()
            self.train_op_Yellowfin = self.optimizer_Yellowfin.minimize(self.loss1,
                                                            var_list=self.weights1 + self.biases1)
            self.train_op_Yellowfin2 = self.optimizer_Yellowfin.minimize(self.loss2,
                                                            var_list=self.weights2 + self.biases2)
            self.final_train = tf.group(self.train_op_Yellowfin,self.train_op_Yellowfin2)
    ...
    def load_NN(self, fileDir, layers):
        ...
        weights2 = []
        biases2 = []
        num_layers1 = len(layers[0])
        num_layers2 = len(layers[1])
        with open(fileDir, 'rb') as f:
            uv_weights1, uv_biases1, uv_weights2, uv_biases2 = pickle.load(f)

            # Stored model must has the same # of layers
            assert num_layers1 == (len(uv_weights1)+1)
            assert num_layers2 == (len(uv_weights2)+1)
            for num in range(0, num_layers1 - 1):
                W = tf.Variable(uv_weights1[num], dtype=tf.float32)
                b = tf.Variable(uv_biases1[num], dtype=tf.float32)
                weights1.append(W)
                biases1.append(b)
                print(" - Load NN parameters successfully...")
            for num in range(0, num_layers2 - 1):
                W = tf.Variable(uv_weights2[num], dtype=tf.float32)
                b = tf.Variable(uv_biases2[num], dtype=tf.float32)
                weights2.append(W)
                biases2.append(b)
                print(" - Load NN parameters successfully...")
    ...
    def net_u1(self, x, y):
        ...
        if use_neural_form ==0:
            u = psips[:,0:1]
            v = psips[:,1:2]
        elif use_neural_form == 1:
            dx = ixlb - xlb
            dy = yub - ylb
            
            axy = (ixlb - x)/(dx)*u_ref + (x - xlb)/(dx)*u_ref + \
            ((yub - y)/dy)*(u_ref - (((ixlb - x)/dx)*u_ref + ((x - xlb)/dx)*u_ref)) + \
            ((y - ylb)/dy)*(u_ref -(((ixlb - x)/dx)*u_ref + ((x - xlb)/dx)*u_ref))
            
            u = axy + psips[:,0:1]*(x - xlb)*(yub - y)*(y - ylb)/(dx*(dy**2))
            
            axy = (ixlb - x)/(dx)*v_ref + (x - xlb)/(dx)*v_ref + \
            ((yub - y)/dy)*(v_ref - (((ixlb - x)/dx)*v_ref + ((x - xlb)/dx)*v_ref)) + \
            ((y - ylb)/dy)*(v_ref -(((ixlb - x)/dx)*v_ref + ((x - xlb)/dx)*v_ref))
    ...
    def net_u2(self, x, y):
        ...
        if use_neural_form ==0:
            ...
            axy = (xub - x)/(dx)*u_ref + (x - ixlb)/(dx)*u_ref + \
            ((yub - y)/dy)*(u_ref - (((xub - x)/dx)*u_ref + ((x - ixlb)/dx)*u_ref)) + \
            ((y - ylb)/dy)*(u_ref -(((xub - x)/dx)*u_ref + ((x - ixlb)/dx)*u_ref))
            
            u = axy + psips[:,0:1]*(x - ixlb)*(yub - y)*(y - ylb)/(dx*(dy**2))
            
            axy = (xub - x)/(dx)*v_ref + (x - ixlb)/(dx)*v_ref + \
            ((yub - y)/dy)*(v_ref - (((xub - x)/dx)*v_ref + ((x - ixlb)/dx)*v_ref)) + \
            ((y - ylb)/dy)*(v_ref -(((xub - x)/dx)*v_ref + ((x - ixlb)/dx)*v_ref))
            
            v = axy + psips[:,1:2]*(x - ixlb)*(yub - y)*(y - ylb)/(dx*(dy**2))
        p = psips[:,2:3]
        s11 = psips[:,3:4]
        s22 = psips[:,4:5]
        s12 = psips[:,5:6]
    ...
    def stressfree_bc(self, x1,y1):
        ...
        u2, v2, p2, s11, s22, s12 = self.net_u2(x1, y1)
        re=self.re
        u_x2 = tf.gradients(u2, x1)[0]
        u_y2 = tf.gradients(u2, y1)[0]

        v_x2 = tf.gradients(v2, x1)[0]
    ...
    def callback(self, loss, loss2):
        ...
        self.loss_rec.append(loss + loss2)
        if self.count % 40 == 0:
    ...
    def viscous_forces_autodiff(self,points):
        ...
        tf_dict = {self.x_CYLD1_tf: x_cyl, self.y_CYLD1_tf: y_cyl}
        
        [u_x_star,
         u_y_star,
         v_x_star,
         v_y_star] = self.sess.run([self.u_x_CYLD_pred1,
                                    self.u_y_CYLD_pred1,
                                    self.v_x_CYLD_pred1,
                                    self.v_y_CYLD_pred1], tf_dict)
    ...
    def viscous_forces_autodiff2(self,points):
        #calculate viscous forces of circle using TF autodiff
        ...
        tf_dict = {self.x_CYLD2_tf: x_cyl, self.y_CYLD2_tf: y_cyl}
        
        [u_x_star,
         u_y_star,
         v_x_star,
         v_y_star] = self.sess.run([self.u_x_CYLD_pred2,
                                    self.u_y_CYLD_pred2,
                                    self.v_x_CYLD_pred2,
                                    self.v_y_CYLD_pred2], tf_dict)
    ...
    def train(self, iter, learning_rate, initial):
        ...
        tf_dict = {self.x_c1_tf: self.x_c1, self.y_c1_tf: self.y_c1, self.x_c2_tf: self.x_c2, self.y_c2_tf: self.y_c2, self.xi1_tf: self.x_i1, self.yi1_tf: self.y_i1, 
                   self.x_CYLD1_tf: self.x_CYLD1, self.y_CYLD1_tf: self.y_CYLD1, self.u_CYLD1_tf: self.u_CYLD1, self.v_CYLD1_tf: self.v_CYLD1,
                   self.x_CYLD2_tf: self.x_CYLD2, self.y_CYLD2_tf: self.y_CYLD2, self.u_CYLD2_tf: self.u_CYLD2, self.v_CYLD2_tf: self.v_CYLD2,
                   self.x_WALL1_tf: self.x_WALL1, self.y_WALL1_tf: self.y_WALL1, self.u_WALL1_tf: self.u_WALL1, self.v_WALL1_tf: self.v_WALL1,  
                   self.x_WALL2_tf: self.x_WALL2, self.y_WALL2_tf: self.y_WALL2, self.u_WALL2_tf: self.u_WALL2, self.v_WALL2_tf: self.v_WALL2,                     
                   self.x_INLET_tf: self.x_INLET, self.y_INLET_tf: self.y_INLET, self.u_INLET_tf: self.u_INLET, self.v_INLET_tf: self.v_INLET,
                   self.x_internal1_tf: self.x_internal1, self.y_internal1_tf: self.y_internal1, self.u_internal1_tf: self.u_internal1, self.v_internal1_tf: self.v_internal1, self.p_internal1_tf: self.p_internal1,
                   self.x_internal2_tf: self.x_internal2, self.y_internal2_tf: self.y_internal2, self.u_internal2_tf: self.u_internal2, self.v_internal2_tf: self.v_internal2, self.p_internal2_tf: self.p_internal2,
                   self.x_OUTLET_tf: self.x_OUTLET, self.y_OUTLET_tf: self.y_OUTLET, self.p_OUTLET_tf: self.p_OUTLET,
                   self.learning_rate: learning_rate, self.wall_weight: initial, self.inlet_weight1: initial, self.inlet_weight2: initial, self.outlet_weight1: initial, self.outlet_weight2: initial, self.cyld_weight: initial}
        ...
        for it in range(iter):
            ...
            if it % 100 == 0:
                """
                if (self.sess.run(self.loss_WALL,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) >=10:
                    tf_dict[self.wall_weight]= 0.1
                if (self.sess.run(self.loss_WALL,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) <=0.1:
                    tf_dict[self.wall_weight]= 10
                if (self.sess.run(self.loss_WALL,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) >=0.1 and (self.sess.run(self.loss_WALL,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) <=10 :
                    tf_dict[self.wall_weight]= 2        
                if (self.sess.run(self.loss_CYLD,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) >=10:
                    tf_dict[self.cyld_weight]= 0.1
                if (self.sess.run(self.loss_CYLD,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) <=0.1:
                    tf_dict[self.cyld_weight]= 10
                if (self.sess.run(self.loss_CYLD,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) >=0.1 and (self.sess.run(self.loss_CYLD,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) <=10 :
                    tf_dict[self.cyld_weight]= 2
                if (self.sess.run(self.loss_INLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) >=10:
                    tf_dict[self.inlet_weight1]= 0.1
                if (self.sess.run(self.loss_INLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) <=0.1:
                    tf_dict[self.inlet_weight1]= 10
                if (self.sess.run(self.loss_INLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) >=0.1 and (self.sess.run(self.loss_INLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) <=10 :
                    tf_dict[self.inlet_weight1]= 2 
                if (self.sess.run(self.loss_INLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) >=10:
                    tf_dict[self.inlet_weight2]= 0.1
                if (self.sess.run(self.loss_INLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) <=0.1:
                    tf_dict[self.inlet_weight2]= 10
                if (self.sess.run(self.loss_INLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) >=0.1 and (self.sess.run(self.loss_INLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) <=10 :
                    tf_dict[self.inlet_weight2]= 2 
                if (self.sess.run(self.loss_OUTLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) >=10:
                    tf_dict[self.outlet_weight1]= 0.1
                if (self.sess.run(self.loss_OUTLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) <=0.1:
                    tf_dict[self.outlet_weight1]= 10
                if (self.sess.run(self.loss_OUTLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) >=0.1 and (self.sess.run(self.loss_OUTLET1,tf_dict) / self.sess.run(self.loss_f1,tf_dict)) <=10 :
                    tf_dict[self.outlet_weight1]= 2
                if (self.sess.run(self.loss_OUTLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) >=10:
                    tf_dict[self.outlet_weight2]= 0.1
                if (self.sess.run(self.loss_OUTLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) <=0.1:
                    tf_dict[self.outlet_weight2]= 10
                if (self.sess.run(self.loss_OUTLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) >=0.1 and (self.sess.run(self.loss_OUTLET2,tf_dict) / self.sess.run(self.loss_f2,tf_dict)) <=10 :
                    tf_dict[self.outlet_weight2]= 2
                """
            ...
            if it == 1000:
                tf_dict[self.learning_rate] = learning_rate/10

            loss_CYLD.append(self.sess.run(self.loss_CYLD1, tf_dict))
            loss_CYLD_2.append(self.sess.run(self.loss_CYLD2, tf_dict))
            loss_WALL.append(self.sess.run(self.loss_WALL1, tf_dict)+self.sess.run(self.loss_WALL2, tf_dict))            
            loss_f1.append(self.sess.run(self.loss_f1, tf_dict))
            loss_INLET.append(self.sess.run(self.loss_INLET, tf_dict))
            loss_internal.append(self.sess.run(self.loss_internal1, tf_dict) + self.sess.run(self.loss_internal2,tf_dict))
            loss_OUTLET.append(self.sess.run(self.loss_OUTLET, tf_dict))
            iterate_count.append(it+1)
            self.loss_rec.append(self.sess.run(self.loss1, tf_dict)+self.sess.run(self.loss2,tf_dict))
        ...
        return loss_WALL, loss_INLET, loss_CYLD, loss_CYLD_2, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface_list
    ...
    def train_bfgs(self, initial):
        ...
        tf_dict = {self.x_c1_tf: self.x_c1, self.y_c1_tf: self.y_c1, self.x_c2_tf: self.x_c2, self.y_c2_tf: self.y_c2, self.xi1_tf: self.x_i1, self.yi1_tf: self.y_i1, 
                   ...
                   self.x_internal2_tf: self.x_internal2, self.y_internal2_tf: self.y_internal2, self.u_internal2_tf: self.u_internal2, self.v_internal2_tf: self.v_internal2, self.p_internal2_tf: self.p_internal2,
                   self.x_OUTLET_tf: self.x_OUTLET, self.y_OUTLET_tf: self.y_OUTLET, self.p_OUTLET_tf: self.p_OUTLET,
                   self.wall_weight: initial, self.inlet_weight1: initial, self.inlet_weight2: initial, self.outlet_weight1: initial, self.outlet_weight2: initial, self.cyld_weight: initial}
    ...
    def pressure_predict(self, x_star, y_star):
        u_star = self.sess.run(self.u1_pred, {self.x1_tf: x_star, self.y1_tf: y_star})
        v_star = self.sess.run(self.v1_pred, {self.x1_tf: x_star, self.y1_tf: y_star})
        p_star = self.sess.run(self.p1_pred, {self.x1_tf: x_star, self.y1_tf: y_star})
        return u_star, v_star, p_star

    def pressure_predict2(self, x_star, y_star):
    ...
    def skinfriction_predict(self, x_star, y_star):
        sxy_star = self.sess.run(self.sxy2_pred, {self.x2_tf: x_star, self.y2_tf: y_star})
        return sxy_star
    ...
    def getloss(self):
        ...
        tf_dict = {self.x_c1_tf: self.x_c1, self.y_c1_tf: self.y_c1, self.x_c2_tf: self.x_c2, self.y_c2_tf: self.y_c2, self.xi1_tf: self.x_i1, self.yi1_tf: self.y_i1, 
                   ...
                   self.x_INLET1_tf: self.x_INLET1, self.y_INLET1_tf: self.y_INLET1, self.u_INLET1_tf: self.u_INLET1, self.v_INLET1_tf: self.v_INLET1,
                   self.x_INLET2_tf: self.x_INLET2, self.y_INLET2_tf: self.y_INLET2, self.u_INLET2_tf: self.u_INLET2, self.v_INLET2_tf: self.v_INLET2,
                   ...
                   self.x_OUTLET1_tf: self.x_OUTLET1, self.y_OUTLET1_tf: self.y_OUTLET1, self.p_OUTLET1_tf: self.p_OUTLET1,
                   self.x_OUTLET2_tf: self.x_OUTLET2, self.y_OUTLET2_tf: self.y_OUTLET2, self.p_OUTLET2_tf: self.p_OUTLET2}
...
def mse(ite,los, los2, cyld, cyld2, interface, outlet, wall):
    plt.plot(ite,los)
    plt.xscale("log")
    plt.yscale("log")
    ...
    plt.close('all')
    error_df=pd.DataFrame({"Iteration":ite, "Loss": los, "Loss2": los2, "Cylinder": cyld, "Cylinder 2": cyld2, "Interface": interface, "Outlet": outlet, "Wall": wall})
...
def postProcess3(filename, xmin, xmax, ymin, ymax, field_ref, field_MIXED, field_diff, s=2, alpha=0.5, marker='o'):
    ...
    if diff == 'absolute':
        ...
        cf = ax[0, 2].scatter(x_diff, y_diff, c=u_diff, vmin=-5e-2, vmax=5e-2, alpha=alpha, 
    ...
    if diff == 'absolute':
        ...
        cf = ax[1, 2].scatter(x_diff, y_diff, c=v_diff, vmin=-5e-2, vmax=5e-2, alpha=alpha, 
                              edgecolors='none', cmap='viridis', marker=marker, s=int(s))
    ...
    if diff == 'absolute':
        ...
        cf = ax[2, 2].scatter(x_diff, y_diff, c=velo_diff, vmin=-5e-2, vmax=5e-2, alpha=alpha, 
                              edgecolors='none', cmap='viridis', marker=marker, s=int(s))
        # cf = ax[1,2].imshow(v_diff, vmin=-1e-2, vmax=1e-2, extent=[xmin,xmax,ymin,ymax], 
        #                 origin='lower', cmap='viridis')
        #circ8 = plt.Circle((0,0), radius=0.5, color='w')
    ...
    if diff == 'absolute':
        ...
        cf = ax[3, 2].scatter(x_diff, y_diff, c=p_diff, vmin=-5e-2, vmax=5e-2, alpha=alpha, 
                              edgecolors='none', cmap='viridis', marker=marker, s=int(s))
...
def pressure_forces(points):
    ...
    circle_ref = np.loadtxt('cyld1.csv',delimiter=',',skiprows=6)
...
def pressure_forces2(points):
    ...
    for i in range(points):
        ...
        theta[i] = i*delta
        x_loc[i] = 0.50*np.cos(theta[i])+3; y_loc[i] = 0.50*np.sin(theta[i])
    
    x_loc = x_loc.reshape(-1, 1); y_loc = y_loc.reshape(-1, 1)
    _, _, p_pred = model.pressure_predict2(x_loc, y_loc)
    ...
    fig = plt.figure()
    error_df=pd.DataFrame({"Theta": theta, "Pressure": p_pred.flatten()})
    error_df.transpose().to_csv('./Non_Dim_Output/Pressure_plot2.csv', index=False)
     
    circle_ref = np.loadtxt('cyld2.csv',delimiter=',',skiprows=6)
    x_circle = circle_ref[:,0:1]
    y_circle = circle_ref[:,1:2]
    p_circle = circle_ref[:,3:4]
    theta1= np.zeros_like(x_circle)

    for i in range(len(x_circle)):
        if y_circle[i]>0 and x_circle[i]>3:
            theta1[i] = np.arctan(y_circle[i]/(x_circle[i]-3))
        elif y_circle[i]>0 and x_circle[i]<3:
            theta1[i] = 3.1415 - np.arctan(y_circle[i]/-(x_circle[i]-3))

    for i in range (len(theta1)):
        if theta1[i] == 0:
            p_circle[i]=0

    p_circle = [i for i in p_circle if i != 0]
    theta1 = [i for i in theta1 if i!= 0]

    plt.plot(theta1, p_circle, label="CFD Reference")
    plt.plot(theta, p_pred, label="PINN Prediction")
    plt.title("Cp vs Theta Cylinder 2")
    plt.xlabel("Theta")
    plt.ylabel("Cp")
    plt.legend()
    plt.savefig('./Non_Dim_Output/pressure_vs_theta2.png',dpi=360)
    plt.show()
    plt.close("all")
...
def skin_friction(points):
    ...
    x_loc = x_loc.reshape(-1, 1); y_loc = y_loc.reshape(-1, 1)
    sxy_pred = model.skinfriction_predict(x_loc, y_loc)
    
    skin_friction = []
    for i in sxy_pred:
        skin_friction.append(i/(0.5*1*1))
        
    error_df=pd.DataFrame({"Theta": theta, "Pressure": skin_friction})
    error_df.transpose().to_csv('./Non_Dim_Output/skin_friction.csv', index=False)
     
    friction_ref = np.loadtxt('Skin.csv',delimiter=',',skiprows=6)
    x_friction = friction_ref[:,0:1]
    y_friction = friction_ref[:,1:2]

    x_friction = (x_friction/180)*np.pi

    plt.clf
    plt.plot(x_friction, y_friction, label="CFD Reference")
    plt.plot(theta, skin_friction, label="PINN Prediction")
    plt.title("Skin_friction vs Theta")
    plt.xlabel("Theta")
    plt.ylabel("Skin_friction")
    plt.legend()
    plt.savefig('./Non_Dim_Output/Skinfriction-vs-theta.png',dpi=360)
    plt.show()
...
def viscous_forces2(points,delta_dist):
    #calculate pressure forces of circle, also plot pressure vs theta
    ...
    for i in range(points):
        ...
        theta[i] = i*delta
        x_loc[i] = 0.50*np.cos(theta[i]) + 3 #prevent dist_x/y = 0
        y_loc[i] = 0.50*np.sin(theta[i]) + 1e-10
        dist_x = np.sign(x_loc[i])*delta_dist; dist_y = np.sign(y_loc[i])*delta_dist
        x1 = x_loc[i] + dist_x; x2 = x_loc[i] + 2.*dist_x
        y1 = y_loc[i] + dist_y; y2 = y_loc[i] + 2.*dist_y
        ...
        u_pred, v_pred, _ = model.pressure_predict2(test_points_x, test_points_y)
        u0 = u_pred[0,0]; v0 = v_pred[0,0]
        ui1 = u_pred[1,0]; vi1 = v_pred[1,0]
        ui2 = u_pred[2,0]; vi2 = v_pred[2,0]
        uj1 = u_pred[3,0]; vj1 = v_pred[3,0]
...
if __name__ == "__main__":
    ...
    uv_layers = [[2] + 12*[40] + [6], [2] + 14*[40] + [6]]  # 2 i/p, 8 layers x 40 neurons each, 6 o/p
    ...
    # WALL = [x, y], u=v=0
    WALL_TOP = [xlb, yub] + [xub-xlb, 0.0] * lhs(2, 3000) #lhs = Latin-Hypercube from pyDOE
    ...
    WALL_BOTTOM = [xlb, ylb] + [xub-xlb, 0.0] * lhs(2, 3000)
    ...
    INLET = [xlb, ylb] + [0.0, yub-ylb] * lhs(2, N_inlet)
    y_INLET = INLET[:,1:2]
    ...
    internal1 = np.array(list(filter(lambda x: ((x[0]<=ixlb)), internal)))
    int1 = np.random.choice(internal1.shape[0], N_internal1, replace=False)    
    internal1 = internal1[int1,:] 

    internal2 = np.array(list(filter(lambda x: ((x[0]>ixlb)), internal)))
    int2 = np.random.choice(internal2.shape[0], N_internal2, replace=False)    
    internal2 = internal2[int2,:] 
    

    # INLET = [x, y], p=0 #or OUTLET?
    OUTLET = [xub, ylb] + [0.0, yub-ylb] * lhs(2, N_outlet)
    x_OUTLET = OUTLET[:,0:1]
    y_OUTLET = OUTLET[:,1:2]
    ...
    WALL1 = np.array(list(filter(lambda x: ((x[0]<=ixlb)), WALL)))
    WALL2 = np.array(list(filter(lambda x: ((x[0]>ixlb)), WALL)))
    if CFD_grid == 0: 
        collo_upper = [xlb,0] + [xub-xlb,yub-0] * lhs(2, 80000)
        collo1_upper = list(filter(lambda x: ((x[0]<=ixlb)), collo_upper)) #Delete points within the subdomain 
        collo1_upper = np.array(collo1_upper)
        idx1 = np.random.choice(collo1_upper.shape[0], int(N_f1/2), replace=False)    
        collo1_upper = collo1_upper[idx1,:] #Randomly selected point from sub domain 1, N_f1 is specified 
        collo1_lower = np.concatenate((collo1_upper[:,0:1],-1*collo1_upper[:,1:2]),1) #create the lower symmetrical
        collo1= np.concatenate((collo1_upper, collo1_lower), 0)
        collo1 = DelCylPT(collo1, xc= 0.0, yc = 0.0, r=0.5)

        collo2_upper = list(filter(lambda x: ((x[0]>ixlb)), collo_upper)) #Delete points within the subdomain 
        collo2_upper = np.array(collo2_upper)
        idx2 = np.random.choice(collo2_upper.shape[0], int(N_f2/2), replace=False)    
        collo2_upper = collo2_upper[idx2,:] #Randomly selected point from sub domain 1, N_f1 is specified 
        collo2_lower = np.concatenate((collo2_upper[:,0:1],-1*collo2_upper[:,1:2]),1) #create the lower symmetrical
        collo2= np.concatenate((collo2_upper, collo2_lower), 0)
        collo2 = DelCylPT(collo2, xc= 3.0, yc = 0.0, r=0.5)
    else:
        dataset = np.loadtxt('Tandem(Re16).csv',delimiter=',',skiprows=6)

        collo_upper = np.concatenate((dataset[:,0:1],dataset[:,1:2]),1)
        collo1_upper = list(filter(lambda x: ((x[0]<=ixlb)), collo_upper))
        collo1 = np.array(collo1_upper)
        #idx1 = np.random.choice(collo1_upper.shape[0], int(N_f1/2), replace=False)    
        #collo1_upper = collo1_upper[idx1,:] #Randomly selected point from sub domain 1, N_f1 is specified 
        #collo1 = DelCylPT(collo1, xc= 0.0, yc = 0.0, r=0.5)

        collo2_upper = list(filter(lambda x: ((x[0]>ixlb)), collo_upper)) #Delete points within the subdomain 
        collo2 = np.array(collo2_upper)
        #idx2 = np.random.choice(collo2_upper.shape[0], int(N_f2/2), replace=False)    
        #collo2_upper = collo2_upper[idx2,:] #Randomly selected point from sub domain 1, N_f1 is specified 
        #collo2 = DelCylPT(collo2, xc= 3.0, yc = 0.0, r=0.5)
    ...
    interface = [ixlb, iylb] + [0, iyub-iylb] * lhs(2, N_interface)

    """
    II = [ixlb, iylb] + [0, iyub-iylb] * lhs(2, 101)
    IO = [ixub, iylb] + [0,iyub-iylb] * lhs(2,101)
    """
    ...
    # Cylinder surface
    r = 0.5
    theta1 = [0.0] + [2*np.pi] * lhs(1, N_cyld1)
    x_CYLD1 = np.multiply(r, np.cos(theta1))+0.0
    y_CYLD1 = np.multiply(r, np.sin(theta1))+0.0
    u_CYLD1 = np.zeros_like(x_CYLD1)
    v_CYLD1 = np.zeros_like(x_CYLD1)
    u_CYLD1[:] = 0.
    v_CYLD1[:] = 0.
    
    r = 0.5
    theta2= [0.0] + [2*np.pi] * lhs(1, N_cyld2)
    x_CYLD2 = np.multiply(r, np.cos(theta2))+3.0
    y_CYLD2 = np.multiply(r, np.sin(theta2))+0.0
    u_CYLD2 = np.zeros_like(x_CYLD2)
    v_CYLD2 = np.zeros_like(x_CYLD2)
    u_CYLD2[:] = 0.
    v_CYLD2[:] = 0.

    CYLD1 = np.concatenate((x_CYLD1, y_CYLD1, u_CYLD1, v_CYLD1),1)

    CYLD2 = np.concatenate((x_CYLD2, y_CYLD2, u_CYLD2, v_CYLD2),1)


    # Visualize the collocation points
    fig, ax = plt.subplots()
    ax.set_aspect('equal')
    plt.scatter(collo1[:,0:1], collo1[:,1:2], marker='o', alpha=0.1 ,color='blue', label = "Domain 1")
    plt.scatter(collo2[:,0:1], collo2[:,1:2], marker="+", alpha =0.3, color="yellow", label="Domain 2")
    plt.scatter(interface[:,0:1],interface[:,1:2], marker="o", alpha=0.1, color="black", label="Interface")
    #plt.scatter(CYLD[:,0:1],CYLD[:,1:2],marker="o", alpha=0.1, color="black")
    # plt.scatter(CYLD[:,0:1], CYLD[:,1:2], marker='o', alpha=0.2 , color='yellow')
    plt.scatter(WALL1[:,0:1], WALL1[:,1:2], marker='o', alpha=0.2 , color='green', label="Boundary")
    plt.scatter(WALL2[:,0:1], WALL2[:,1:2], marker='o', alpha=0.2 , color='green')
    plt.scatter(OUTLET[:, 0:1], OUTLET[:, 1:2], marker='o', alpha=0.2, color='green')
    plt.scatter(CYLD1[:, 0:1], CYLD1[:, 1:2], marker='o', alpha=0.2, color='red')
    plt.scatter(CYLD2[:, 0:1], CYLD2[:, 1:2], marker='o', alpha=0.2, color='red')
    ...
    with tf.device('/device:GPU:0'):
        ...
        model = XPINN_laminar_flow(collo1, collo2, interface, INLET, OUTLET, internal1 , internal2, CYLD1, CYLD2, WALL1, WALL2, uv_layers, lb, ub, ExistModel = use_old_model, uvDir = 'uvNN.pickle')

        start_time = time.time()
        
        if debug == 0:
            if use_old_model==1:
                loss_WALL, loss_INLET, loss_CYLD, loss_CYLD_2, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface = model.train(iter=5000, learning_rate=5e-4, initial=beta)
            else:
                loss_WALL, loss_INLET, loss_CYLD,loss_CYLD_2, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface = model.train(iter=10000, learning_rate=5e-4, initial=beta)
        
        elif debug == 1:
            
            loss_WALL, loss_INLET, loss_CYLD,loss_CYLD_2, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface  = model.train(iter=60, learning_rate=5e-4, initial=beta)
        
        model.train_bfgs(initial=beta)
        ...
        if data_ref == 'fluent':
            ...
            field_ref = np.loadtxt('Tandem(Re16).csv',delimiter=',',skiprows=6)
            x_ref = field_ref[:,0:1]
            y_ref = field_ref[:,1:2]
            u_ref = field_ref[:,4:5]
            v_ref = field_ref[:,5:6]
        ...
        x_PINN1 = np.linspace(xlb, ixlb, 501)
        y_PINN1 = np.linspace(ylb, yub, 201)
        x_PINN1, y_PINN1 = np.meshgrid(x_PINN1, y_PINN1)
        x_PINN1 = (x_PINN1.flatten()[:, None])
        y_PINN1 = (y_PINN1.flatten()[:, None])  
        dst = ((x_PINN1-0.0)**2+(y_PINN1-0.0)**2)**0.5
        x_PINN1 = x_PINN1[dst >= 0.5]
        y_PINN1 = y_PINN1[dst >= 0.5]
        x_PINN1 = (x_PINN1.flatten()[:, None])
        y_PINN1 = (y_PINN1.flatten()[:, None])        
        x_star1 = np.concatenate((x_PINN1, y_PINN1),1)

        x_PINN2 = np.linspace(ixlb, xub, 501)
        y_PINN2 = np.linspace(ylb, yub, 201)
        x_PINN2, y_PINN2 = np.meshgrid(x_PINN2, y_PINN2)
        x_PINN2 = x_PINN2.flatten()[:, None]
        y_PINN2 = y_PINN2.flatten()[:, None]
        dst2 = ((x_PINN2-3.0)**2+(y_PINN2-0.0)**2)**0.5
        x_PINN2 = x_PINN2[dst2 >= 0.5]
        y_PINN2 = y_PINN2[dst2 >= 0.5]
        x_PINN2 = x_PINN2.flatten()[:, None]
        y_PINN2 = y_PINN2.flatten()[:, None]
        ...
        x_int1 = np.linspace(0.5, ixlb, 51)
        y_int1 = np.zeros_like(x_int1)
        t_int1 = np.column_stack((x_int1, y_int1))
        x_int2 = np.linspace(ixlb, 2.5, 51)
        y_int2 = np.zeros_like(x_int2)
        t_int2 = np.column_stack((x_int2, y_int2))
        u_int1, v_int1, p_int1, u_int2, v_int2, p_int2 = model.predict(t_int1, t_int2)
        int_MIXED1 = np.concatenate((t_int1, u_int1, v_int1, p_int1),1)
        int_MIXED2 = np.concatenate((t_int2, u_int2, v_int2, p_int2),1)
        int_MIXED = np.concatenate((int_MIXED1, int_MIXED2),0)
        df_int = pd.DataFrame(int_MIXED)
        df_int.to_csv("int_mixed.csv", index=False)
        Cov_int = pd.read_csv("int_mixed.csv", names=["x", "y", "u", "v","p"])
        Cov_int.to_csv('int_mixed.csv',index=False)
        lines_int = open('int_mixed.csv', 'r').readlines()
        del lines_int[1]
        open('int_mixed.csv', 'w').writelines(lines_int)
        ...
        mse(ite=iterate_count, los=loss_list, los2=loss_list2, cyld = loss_CYLD, cyld2 = loss_CYLD_2, interface = loss_interface, outlet=loss_OUTLET, wall=loss_WALL)        
        if diff == 'absolute':
    ...
    fx_p2,fy_p2 = pressure_forces2(surface_pts)
    fx_v,fy_v = model.viscous_forces_autodiff(surface_pts)
    fx_v2,fy_v2 = model.viscous_forces_autodiff2(surface_pts)
    ...
    print(fx_p,fx_v,fx1)
    print(fy_p,fy_v,fy1)
    print(fx_p2,fx_v2,fx2)
    print(fy_p2,fy_v2,fy2)
