...
class XPINN_laminar_flow:
    ...
    def __init__(self, collo1, collo2, interface, INLET, OUTLET, internal1, internal2, CYLD1, CYLD2, WALL1, WALL2, uv_layers, lb, ub, ExistModel, uvDir):
        ...
        # Initialize NNs
        if ExistModel== 0 :
            self.weights1, self.biases1, self.a1 = self.initialize_NN(self.layer1)
            self.weights2, self.biases2, self.a2 = self.initialize_NN(self.layer2)
        else:
            print("Loading uv NN ...")
        ...
        if use_internal == 0:
            ...
            self.loss2 = self.loss_f2 + 4*(self.loss_CYLD2 + self.loss_WALL2+ self.loss_OUTLET) + 20*self.loss_discontinuous2 + 10*self.loss_continuity
            #self.loss1 = self.loss_f1 + 2*(self.loss_INLET + self.loss_CYLD1+ self.loss_WALL1) + 10*self.loss_continuity
            #self.loss2 = self.loss_f2 + 2*(self.loss_CYLD2 + self.loss_WALL2+ self.loss_OUTLET) + 10*self.loss_continuity
        ...
        if debug == 0:
            ...
            self.optimizer1 = tf.contrib.opt.ScipyOptimizerInterface(self.loss1 + self.loss2,
                                                                    var_list=self.weights1 + self.biases1 +self.weights2 +self.biases2 + self.a1 +self.a2,
                                                                    method='L-BFGS-B',
        ...
        if optimizer ==0: 
            ...
            self.train_op_Adam1 = self.optimizer_Adam.minimize(loss=self.loss1,
                                                            var_list=self.weights1 + self.biases1 + self.a1)
            self.train_op_Adam2 = self.optimizer_Adam2.minimize(self.loss2,
                                                            var_list=self.weights2 + self.biases2 + self.a2)
    ...
    def neural_net(self, X, weights, biases, a):
        ...
        if activation_fn == 0:
            ...
            for l in range(0, num_layers - 2):
                W = weights[l]
                b = biases[l]
                H = tf.tanh(20*a[0]*tf.add(tf.matmul(H, W), b))
    ...
    def net_u1(self, x, y):
        psips = self.neural_net(tf.concat([x, y], 1), self.weights1, self.biases1, self.a1)
    ...
    def net_u2(self, x, y):
        psips = self.neural_net(tf.concat([x, y], 1), self.weights2, self.biases2, self.a2)
    ...
    def train(self, iter, learning_rate, initial):
        ...
        for it in range(iter):
            ...
            loss_CYLD.append(self.sess.run(self.loss_CYLD1, tf_dict)+self.sess.run(self.loss_CYLD2, tf_dict))
            ...
            a1.append(self.sess.run(self.a1))
            a2.append(self.sess.run(self.a2))

        # return loss_CYLD, loss_WALL, loss_INLET, loss_OUTLET, loss_f, self.loss
        return loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface_list, a1, a2
...
def mse(ite,los, los2, cyld, interface, outlet, wall, a1, a2):
    plt.plot(ite,los)
    plt.xscale("log")
    plt.yscale("log")
    ...
    plt.close('all')
    error_df=pd.DataFrame({"Iteration":ite, "Loss": los, "Loss2": los2, "Cylinder": cyld, "Interface": interface, "Outlet": outlet, "Wall": wall, "a1": a1, "a2":a2})
...
if __name__ == "__main__":
    ...
    uv_layers = [[2] + 8*[40] + [6], [2] + 8*[40] + [6]]  # 2 i/p, 8 layers x 40 neurons each, 6 o/p
    ...
    # WALL = [x, y], u=v=0
    WALL_TOP = [xlb, yub] + [xub-xlb, 0.0] * lhs(2, 401) #lhs = Latin-Hypercube from pyDOE
    ...
    WALL_BOTTOM = [xlb, ylb] + [xub-xlb, 0.0] * lhs(2, 401)
    ...
    fig, ax = plt.subplots()
    ax.set_aspect('equal')
    plt.scatter(collo1[:,0:1], collo1[:,1:2], marker='o', alpha=0.1 ,color='blue')
    plt.scatter(collo2[:,0:1], collo2[:,1:2], marker="+", alpha =0.3, color="yellow")
    plt.scatter(interface[:,0:1],interface[:,1:2], marker="o", alpha=0.1, color="black")
    ...
    plt.scatter(WALL1[:,0:1], WALL1[:,1:2], marker='o', alpha=0.2 , color='green')
    ...
    with tf.device('/device:GPU:0'):
        ...
        if debug == 0:
            if use_old_model==1:
                loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface = model.train(iter=5000, learning_rate=5e-4, initial=beta)
            else:
                loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface, a1, a2 = model.train(iter=1000, learning_rate=5e-4, initial=beta)
        
        elif debug == 1:
            
            loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface, a1, a2  = model.train(iter=500, learning_rate=5e-4, initial=beta)
        ...
        x_PINN1 = np.linspace(xlb, ixlb, 251)
        ...
        x_PINN2 = np.linspace(ixlb, xub, 251)
        y_PINN2 = np.linspace(ylb, yub, 101)
        ...
        mse(ite=iterate_count, los=loss_list, los2=loss_list2, cyld = loss_CYLD, interface = loss_interface, outlet=loss_OUTLET, wall=loss_WALL, a1 = a1, a2=a2)        
        if diff == 'absolute':
