...
class XPINN_laminar_flow:
    ...
    def __init__(self, collo1, collo2, interface, INLET1, INLET2, internal1, internal2, OUTLET1, OUTLET2, CYLD, WALL, uv_layers, lb, ub, ExistModel=0, uvDir=''):
        ...
        if use_internal == 0:
            ...
            self.loss1 = self.loss_f1 + 2*(self.loss_INLET1+ self.loss_OUTLET1 + self.loss_WALL) + 2*self.loss_discontinuous1 + self.loss_continuity
            self.loss2 = self.loss_f2 + 2*(self.loss_CYLD + self.loss_INLET2 + self.loss_OUTLET2) + 2*self.loss_discontinuous2 + self.loss_continuity
    ...
    def train(self, iter, learning_rate):
        ...
        for it in range(iter):
            ...
            adaptive_a1.append(self.sess.run(self.a1))
            adaptive_a2.append(self.sess.run(self.a2))
        ...
        return loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface_list, adaptive_a1, adaptive_a2
...
def mse(ite,los, los2, cyld, interface, outlet, wall, a1, a2):
    ...
    plt.close('all')
    error_df=pd.DataFrame({"Iteration":ite, "Loss": los, "Loss2": los2, "Cylinder": cyld, "Interface": interface, "Outlet": outlet, "Wall": wall, "Adaptive 1": a1, "Adaptive 2" : a2})
...
def skin_friction(points):
    ...
    skin_friction = []
    for i in sxy_pred:
        skin_friction.append(i/(0.5*0.025*1))
    ...
    circle_ref = np.loadtxt('Skin.csv',delimiter=',',skiprows=6)
    x_circle = circle_ref[:,0:1]
    y_circle = circle_ref[:,1:2]
    ...
    plt.plot(x_circle, y_circle, label="CFD Reference")
    plt.plot(theta, sxy_pred, label="PINN Prediction")
...
if __name__ == "__main__":
    ...
    uv_layers = [[2] + 4*[30] + [6], [2] + 6*[40] + [6]]  # 2 i/p, 8 layers x 40 neurons each, 6 o/p
    ...
    plt.scatter(WALL[:,0:1], WALL[:,1:2], marker='o', alpha=0.2 , color='green', label="Boundary")
    ...
    with tf.device('/device:GPU:0'):
        ...
        if debug == 0:
            ...
            loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface, adaptive_a1, adaptive_a2 = model.train(iter=10000, learning_rate=5e-4)
        
        elif debug == 1:
            
            loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface, adaptive_a1, adaptive_a2  = model.train(iter=60, learning_rate=5e-4)
        ...
        mse(ite=iterate_count, los=loss_list, los2=loss_list2, cyld = loss_CYLD, interface = loss_interface, outlet=loss_OUTLET, wall=loss_WALL, a1= adaptive_a1, a2=adaptive_a2 )        
        if diff == 'absolute':
