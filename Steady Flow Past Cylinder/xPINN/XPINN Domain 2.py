...
class XPINN_laminar_flow:
    ...
    def __init__(self, collo1, collo2, interface, INLET1, INLET2, internal1, internal2, OUTLET1, OUTLET2, CYLD, WALL, uv_layers, lb, ub, ExistModel, uvDir):
        ...
        self.x_INLET1 = INLET1[:, 0:1]
        self.y_INLET1 = INLET1[:, 1:2]
        self.u_INLET1 = INLET1[:, 2:3]
        self.v_INLET1 = INLET1[:, 3:4]

        self.x_INLET2 = INLET2[:, 0:1]
        self.y_INLET2 = INLET2[:, 1:2]
        self.u_INLET2 = INLET2[:, 2:3]
        self.v_INLET2 = INLET2[:, 3:4]
        ...
        self.x_OUTLET1 = OUTLET1[:, 0:1]
        self.y_OUTLET1 = OUTLET1[:, 1:2]
        self.p_OUTLET1 = OUTLET1[:, 2:3]
    
        self.x_OUTLET2 = OUTLET2[:, 0:1]
        self.y_OUTLET2 = OUTLET2[:, 1:2]
        self.p_OUTLET2 = OUTLET2[:, 2:3]
        ...
        self.x_OUTLET1_tf = tf.placeholder(tf.float32, shape=[None, self.x_OUTLET1.shape[1]])
        self.y_OUTLET1_tf = tf.placeholder(tf.float32, shape=[None, self.y_OUTLET1.shape[1]])
        self.p_OUTLET1_tf = tf.placeholder(tf.float32, shape=[None, self.p_OUTLET1.shape[1]])

        self.x_OUTLET2_tf = tf.placeholder(tf.float32, shape=[None, self.x_OUTLET2.shape[1]])
        self.y_OUTLET2_tf = tf.placeholder(tf.float32, shape=[None, self.y_OUTLET2.shape[1]])
        self.p_OUTLET2_tf = tf.placeholder(tf.float32, shape=[None, self.p_OUTLET2.shape[1]])

        self.x_INLET1_tf = tf.placeholder(tf.float32, shape=[None, self.x_INLET1.shape[1]])
        self.y_INLET1_tf = tf.placeholder(tf.float32, shape=[None, self.y_INLET1.shape[1]])
        self.u_INLET1_tf = tf.placeholder(tf.float32, shape=[None, self.u_INLET1.shape[1]])
        self.v_INLET1_tf = tf.placeholder(tf.float32, shape=[None, self.v_INLET1.shape[1]])

        self.x_INLET2_tf = tf.placeholder(tf.float32, shape=[None, self.x_INLET2.shape[1]])
        self.y_INLET2_tf = tf.placeholder(tf.float32, shape=[None, self.y_INLET2.shape[1]])
        self.u_INLET2_tf = tf.placeholder(tf.float32, shape=[None, self.u_INLET2.shape[1]])
        self.v_INLET2_tf = tf.placeholder(tf.float32, shape=[None, self.v_INLET2.shape[1]])
        ...
        self.u_INLET1_pred, self.v_INLET1_pred, _, _, _, _ = self.net_u1(self.x_INLET1_tf, self.y_INLET1_tf) #For inlet in subdomain 1
        self.u_INLET2_pred, self.v_INLET2_pred, _, _, _, _ = self.net_u2(self.x_INLET2_tf, self.y_INLET2_tf) #For inlet in subdomain 2
        ...
        self.p_bc1, self.p_bc2, self.p_bc3, self.p_bc4 =self.stressfree_bc(self.x_OUTLET1_tf, self.y_OUTLET1_tf, self.x_OUTLET2_tf, self.y_OUTLET2_tf)
        self.wall_bc = self.wall_stress(self.x_WALL_tf, self.y_WALL_tf)

        _, _, self.p_OUTLET1_pred, self.s11_OUTLET1_pred, _, self.s12_OUTLET1_pred = self.net_u1(self.x_OUTLET1_tf, self.y_OUTLET1_tf) #Outlet domain 1
        _, _, self.p_OUTLET2_pred, self.s11_OUTLET2_pred, _, self.s12_OUTLET2_pred = self.net_u2(self.x_OUTLET2_tf, self.y_OUTLET2_tf) #Outlet domain 2
        ...
        self.loss_INLET1 = tf.reduce_mean(tf.square(self.u_INLET1_pred-self.u_INLET1_tf)) \
                         + tf.reduce_mean(tf.square(self.v_INLET1_pred-self.v_INLET1_tf))

        self.loss_INLET2 = tf.reduce_mean(tf.square(self.u_INLET2_pred-self.u_INLET2_tf)) \
                         + tf.reduce_mean(tf.square(self.v_INLET2_pred-self.v_INLET2_tf))
        ...
        #self.loss_OUTLET1 = tf.reduce_mean(tf.square(self.p_bc1)) + tf.reduce_mean(tf.square(self.p_bc2)) #The stress free boundary condition
        #self.loss_OUTLET2 = tf.reduce_mean(tf.square(self.p_bc3)) + tf.reduce_mean(tf.square(self.p_bc4))
        self.loss_OUTLET1 = tf.reduce_mean(tf.square(self.p_OUTLET1_pred))
        self.loss_OUTLET2 = tf.reduce_mean(tf.square(self.p_OUTLET2_pred))
        ...
        if use_internal == 0:
            ...
            self.loss1 = self.loss_f1 + (self.inlet_weight1*self.loss_INLET1+ self.outlet_weight1*self.loss_OUTLET1 + self.wall_weight*self.loss_WALL) + 2*self.loss_discontinuous1 + self.loss_continuity
            self.loss2 = self.loss_f2 + (self.cyld_weight*self.loss_CYLD + self.inlet_weight2*self.loss_INLET2 + self.outlet_weight2*self.loss_OUTLET2) + 2*self.loss_discontinuous2 + self.loss_continuity
    ...
    def stressfree_bc(self, x1,y1, x2 ,y2):
        ...
        u2, v2, p2, s11, s22, s12 = self.net_u2(x2, y2)
        re=self.re
        u_x2 = tf.gradients(u2, x2)[0]
        u_y2 = tf.gradients(u2, y2)[0]

        v_x2 = tf.gradients(v2, x2)[0]

        #Sxx
        bc_3 = (2/re)*(u_x2) - p2
        bc_4 = (1/re)*(u_y2 + v_x2)
        return bc_1, bc_2 , bc_3, bc_4
    ...
    def train(self, iter, learning_rate, initial):
        ...
        for it in range(iter):
            ...
            loss_INLET.append(self.sess.run(self.loss_INLET1, tf_dict) + self.sess.run(self.loss_INLET2,tf_dict))
            loss_internal.append(self.sess.run(self.loss_internal1, tf_dict) + self.sess.run(self.loss_internal2,tf_dict))
            loss_OUTLET.append(self.sess.run(self.loss_OUTLET1, tf_dict) +self.sess.run(self.loss_OUTLET2, tf_dict))
...
def mse(ite,los, los2, cyld, interface, outlet, wall):
    plt.plot(ite,los)
    plt.xscale("log")
    plt.yscale("log")
    ...
    plt.close('all')
    error_df=pd.DataFrame({"Iteration":ite, "Loss": los, "Loss2": los2, "Cylinder": cyld, "Interface": interface, "Outlet": outlet, "Wall": wall})
...
if __name__ == "__main__":
    ...
    INLET = [xlb, ylb] + [0.0, yub-ylb] * lhs(2, 5000)
    y_INLET = INLET[:,1:2]
    ...
    INLET1 = np.array(list(filter(lambda x: ((x[1]<=iylb or x[1]>=iyub)), INLET)))
    in1 = np.random.choice(INLET1.shape[0], N_inlet, replace=False)    
    INLET1 = INLET1[in1,:] 
    INLET2 = np.array(list(filter(lambda x: ((iylb<=x[1]<=iyub)), INLET)))
    in2 = np.random.choice(INLET1.shape[0], N_inlet, replace=False)    
    INLET2 = INLET2[in2,:] 
    ...
    internal1 = np.array(list(filter(lambda x: ((x[1]<=iylb or x[1]>=iyub)), internal)))
    int1 = np.random.choice(internal1.shape[0], N_internal1, replace=False)    
    internal1 = internal1[int1,:] 

    internal2 = np.array(list(filter(lambda x: ((iylb<=x[1]<=iyub)), internal)))
    int2 = np.random.choice(internal2.shape[0], N_internal2, replace=False)    
    internal2 = internal2[int2,:] 
    

    # INLET = [x, y], p=0 #or OUTLET?
    OUTLET = [xub, ylb] + [0.0, yub-ylb] * lhs(2, 5000)
    x_OUTLET = OUTLET[:,0:1]
    y_OUTLET = OUTLET[:,1:2]
    p_OUTLET = np.zeros_like(y_OUTLET)
    p_OUTLET[:] = 0.
    OUTLET = np.concatenate((OUTLET, p_OUTLET), 1)
    OUTLET1 = np.array(list(filter(lambda x: ((x[1]<=iylb or x[1]>=iyub)), OUTLET)))
    out1 = np.random.choice(OUTLET1.shape[0], N_outlet, replace=False)    
    OUTLET1 = OUTLET1[out1,:] 
    OUTLET2 = np.array(list(filter(lambda x: ((iylb<=x[1]<=iyub)), OUTLET)))
    out2 = np.random.choice(OUTLET2.shape[0], N_outlet, replace=False)    
    OUTLET2 = OUTLET2[out2,:] 
    ...
    collo1_upper = [xlb,0] + [xub-xlb,yub-0] * lhs(2, 80000)
    collo1_upper = list(filter(lambda x: ((x[1]<=iylb or x[1]>=iyub)), collo1_upper)) #Delete points within the subdomain 
    ...
    """
    interface= np.concatenate((ITOP,IBOT),0) #Interface coordinates 

    #Subdomain 2
    # Cylinder surface
    r = 0.5
    theta = [0.0] + [2*np.pi] * lhs(1, N_cyld)
    ...
    collo2_upper = [ixlb, 0] + [ixub-ixlb,iyub-0] * lhs(2, int(N_f2/2))   #x,y coord for entire domain
    ...
    plt.scatter(OUTLET1[:, 0:1], OUTLET1[:, 1:2], marker='o', alpha=0.2, color='green')
    plt.scatter(OUTLET2[:, 0:1], OUTLET2[:, 1:2], marker='o', alpha=0.2, color='red')
    plt.scatter(INLET1[:, 0:1], INLET1[:, 1:2], marker='o', alpha=0.2, color='green')
    plt.scatter(INLET2[:, 0:1], INLET2[:, 1:2], marker='o', alpha=0.2, color='red')
    plt.scatter(internal1[:, 0:1], internal1[:, 1:2], marker='o', alpha=0.2, color='green')
    plt.scatter(internal2[:, 0:1], internal2[:, 1:2], marker='o', alpha=0.2, color='green')
    plt.show()
    ...
    with tf.device('/device:GPU:0'):
        ...
        model = XPINN_laminar_flow(collo1, collo2, interface, INLET1, INLET2, internal1 , internal2, OUTLET1, OUTLET2, CYLD, WALL, uv_layers, lb, ub, ExistModel = use_old_model, uvDir = 'uvNN.pickle')

        start_time = time.time()
        
        if debug == 0:
            if use_old_model==1:
                loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface = model.train(iter=1000, learning_rate=5e-4, initial=beta)
            else:
                loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface = model.train(iter=10000, learning_rate=5e-4, initial=beta)
        
        elif debug == 1:
            
            loss_WALL, loss_INLET, loss_CYLD, loss_internal, loss_OUTLET, loss_f1, iterate_count, loss_list, loss_list2, loss_interface  = model.train(iter=60, learning_rate=5e-4, initial=beta)
        ...
        if data_ref == 'fluent':
            if Re==40:
                ...
                p_ref = field_ref[:,3:4]
            if Re==5:
                field_ref = np.loadtxt('Re=5.csv',delimiter=',',skiprows=6)
                x_ref = field_ref[:,0:1]
                y_ref = field_ref[:,1:2]
                u_ref = field_ref[:,4:5]
                v_ref = field_ref[:,5:6]
        ...
        x_star1 = list(filter(lambda x: ((x[1]<=iylb or x[1]>=iyub)), x_star1))
        ...
        mse(ite=iterate_count, los=loss_list, los2=loss_list2, cyld = loss_CYLD, interface = loss_interface, outlet=loss_OUTLET, wall=loss_WALL)        
        if diff == 'absolute':
