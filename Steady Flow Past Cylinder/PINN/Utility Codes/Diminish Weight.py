...
class PINN_laminar_flow:
    ...
    def __init__(self, Collo, INLET, internal, OUTLET_n, OUTLET_d, CYLD, TOP_BOTTOM, uv_layers, lb, ub, ExistModel=0, uvDir=''):
        ...
        self.diminishing_weight =tf.placeholder(tf.float32, shape=[])
        ...
        if use_internal == 0:
            if use_neural_form==0:
                self.loss_bc = (self.loss_CYLD + self.loss_TOP_BOTTOM + self.loss_INLET + self.loss_stress_free)
                self.loss = self.loss_f + self.diminishing_weight*(self.loss_CYLD + self.loss_TOP_BOTTOM + self.loss_INLET + self.loss_stress_free)
    ...
    def train(self, iter, learning_rate, initial):
        ...
        tf_dict = {self.x_c_tf: self.x_c, self.y_c_tf: self.y_c,
                   ...
                   self.learning_rate: learning_rate, self.diminishing_weight: initial}
        ...
        for it in range(iter):
            ...
            if it % 100 == 0 and it != 0:
                tf_dict[self.diminishing_weight] = initial/it #this is to update tf_dict every 100 iterations
                current_weight = self.sess.run(self.diminishing_weight, tf_dict) #use self.sess.run to obtain the current diminishing weight

            if it % 100 == 0:
                loss_value = self.sess.run(self.loss, tf_dict)
                loss_bc = self.sess.run(self.loss_bc, tf_dict)
                loss_residual = self.sess.run(self.loss_f,tf_dict)
                validation = loss_residual + current_weight*loss_bc
                print('It: %d, Loss: %.3e, Current: %.3e, Validate: %.3e' %
                      (it, loss_value, current_weight, validation)) #validation has to be same as loss value to ensure code works 
                weight_list.append(current_weight) #Stores the weights 
        ...
        loss_OUTLET_dirichlet,loss_OUTLET_neumann, loss_f, self.loss, iterate_count, loss_list, current_weight #return every variable which can be accessed in model.train
    ...
    def train_bfgs(self, initial):
        ...
        tf_dict = {self.x_c_tf: self.x_c, self.y_c_tf: self.y_c,
                   ...
                   self.x_OUTLET_d_tf: self.x_OUTLET_d, self.y_OUTLET_d_tf: self.y_OUTLET_d, self.p_OUTLET_d_tf: self.p_OUTLET_d,
                   self.x_OUTLET_n_tf: self.x_OUTLET_n, self.y_OUTLET_n_tf: self.y_OUTLET_n, self.p_OUTLET_n_tf: self.p_OUTLET_n,
                   self.diminishing_weight: initial}
...
def mse(ite,los,cylinder):
    ...
    plt.close('all')
    error_df=pd.DataFrame({"Iteration":ite, "Loss": los, "Cylinder": cylinder})
    ...
    with tf.device('/device:GPU:0'):
        ...
        if debug == 0:
            if optimizer==1:
                ...
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, current_weight = model.train(iter=6500, learning_rate=0.0005, initial=10) #Take the current weight
            else:
                loss_CYLD, loss_CYLD_neumann, loss_internal, loss_OUTLET_dirichlet, \
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, current_weight = model.train(iter=10000, learning_rate=0.0005, initial=10)
        
        elif debug == 1:
            
                loss_CYLD, loss_CYLD_neumann, loss_internal, loss_OUTLET_dirichlet, \
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, current_weight  = model.train(iter=60, learning_rate=0.0005, initial = 10)
        
        model.train_bfgs(initial=current_weight) #feed current weight into the BFGS then use the same method as the train. 
        ...
        mse(ite=iterate_count, los=loss_list, cylinder=loss_CYLD)      
        if diff == 'absolute':
            if CFD_grid ==1:
                u_diff = u_PINN - u_ref
                v_diff = v_PINN - v_ref
