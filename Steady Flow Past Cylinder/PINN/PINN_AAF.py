...
class PINN_laminar_flow:
    ...
    def __init__(self, Collo, INLET, internal, OUTLET_n, OUTLET_d, CYLD, TOP_BOTTOM, uv_layers, lb, ub, ExistModel=0, uvDir=''):
        ...
        # Initialize NNs
        if ExistModel== 0 :
            self.uv_weights, self.uv_biases, self.uv_A = self.initialize_NN(self.uv_layers)
        else:
            print("Loading uv NN ...")
            self.uv_weights, self.uv_biases, self.uv_A = self.load_NN(uvDir, self.uv_layers)
        ...
        #placeholder for adaptive constants
        ...
        if use_internal == 0:
            if use_neural_form==0:
                self.loss_bc = self.adaptive_constant_tf*(self.loss_CYLD + self.loss_TOP_BOTTOM + self.loss_INLET + self.loss_stress_free)
                self.loss = self.loss_f + self.adaptive_constant_tf*(self.loss_CYLD + self.loss_TOP_BOTTOM + self.loss_INLET + self.loss_stress_free)
            #self.loss = self.loss_f + 0* self.loss_internal + \
            #    2*(self.loss_TOP_BOTTOM + self.loss_INLET + self.loss_OUTLET)
            else:

                self.loss = self.loss_f + self.adaptive_constant_tf*(self.loss_CYLD + self.loss_CYLD_neumann + self.loss_OUTLET_neumann + self.loss_OUTLET_dirichlet)
                self.loss_bc = self.adaptive_constant_tf*(self.loss_CYLD + self.loss_CYLD_neumann + self.loss_OUTLET_neumann + self.loss_OUTLET_dirichlet)
            ...
            self.loss = self.loss_f + self.adaptive_constant_tf*(self.loss_CYLD + self.loss_CYLD_neumann + \
                     self.loss_OUTLET_neumann + \
                        self.loss_OUTLET_dirichlet) + self.loss_internal
            self.loss_bc = self.adaptive_constant_tf*(self.loss_CYLD + self.loss_CYLD_neumann + self.loss_OUTLET_neumann + self.loss_OUTLET_dirichlet)
        ...
        if debug == 0:
            ...
            self.optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.loss,
                                                                    var_list=self.uv_weights + self.uv_biases + self.uv_A,
        ...
        self.learning_rate=tf.train.exponential_decay(starter_learning_rate,self.global_step, 1000,0.9,staircase=True)
        self.optimizer_Adam = tf.train.AdamOptimizer(learning_rate = starter_learning_rate)
        ...
        self.train_op_Yellowfin = self.optimizer_Yellowfin.minimize(self.loss,
                                                          var_list=self.uv_weights + self.uv_biases + self.uv_A)
        #Implementing Adaptive Weights
        #Gradient Storage
        self.grad_res = []
        self.grad_bcs = []
        for i in range(len(self.uv_layers) -1):
            self.grad_res.append(tf.gradients(self.loss_f, self.uv_weights[i])[0])
            self.grad_bcs.append(tf.gradients(self.loss_bc, self.uv_weights[i])[0])

        #Compute and store adaptive constant
    ...
    def save_NN(self, fileDir):
        ...
        uv_A = self.sess.run(self.uv_A)
        

        with open(fileDir, 'wb') as f:
            pickle.dump([uv_weights, uv_biases, uv_A], f)
    ...
    def load_NN(self, fileDir, layers):
        ...
        with open(fileDir, 'rb') as f:
            uv_weights, uv_biases, uv_A = pickle.load(f)
            ...
            for num in range(0, num_layers - 1):
                ...
                a = tf.Variable(uv_A[num], dtype=tf.float32)
                weights.append(W)
                biases.append(b)
                adaptive.append(a)
                print(" - Load NN parameters successfully...")
        return weights, biases, adaptive
    ...
    def neural_net(self, X, weights, biases,A):
        ...
        if activation_fn == 0:
            ...
            for l in range(0, num_layers - 2):
                W = weights[l]
                b = biases[l]
                H = tf.tanh(10*A[0]*tf.add(tf.matmul(H, W), b))
            ...
            for l in range(0, num_layers - 2):
                ...
                H = tf.nn.relu(10*A[l]*tf.add(tf.matmul(H, W), b))
            ...
            for l in range(0, num_layers - 2):
                ...
                H = tf.nn.leaky_relu(10*A[l]*tf.add(tf.matmul(H, W), b))
            ...
            for l in range(0, num_layers - 2):
                ...
                H = tf.nn.selu(10*A[l]*tf.add(tf.matmul(H, W), b))
    ...
    def net_uv(self, x, y):
        psips = self.neural_net(tf.concat([x, y], 1), self.uv_weights, self.uv_biases, self.uv_A)
    ...
    def train(self, iter, learning_rate):
        ...
        for it in range(iter):
            ...
            if it % 40 == 0:
                ...
                loss_bcs_value, loss_res_value = self.sess.run([self.loss_bc, self.loss],tf_dict)

                adaptive_constant_value=self.sess.run(self.adaptive_constant, tf_dict)
                self.adaptive_constant_val = adaptive_constant_value* (1-self.beta) + self.beta * self.adaptive_constant_val
                tf_dict[self.adaptive_constant_tf] = self.adaptive_constant_val
                self.adaptive_constant_log.append(self.adaptive_constant_val)
                print('It: %d, Loss: %.3e, Adaptive Constant: %.2f' %
                      (it, loss_value, self.adaptive_constant_val))

            adaptive_a.append(self.sess.run(self.uv_A))
            ...
            aw_list.append(self.adaptive_constant_val)

        # return loss_CYLD, loss_TOP_BOTTOM, loss_INLET, loss_OUTLET, loss_f, self.loss
        return loss_CYLD, loss_CYLD_neumann, loss_internal, \
        loss_OUTLET_dirichlet,loss_OUTLET_neumann, loss_f, self.loss, iterate_count, loss_list, adaptive_a, aw_list
...
def mse(ite,los,cylinder, a, aw_list):
    plt.figure(3)
    ...
    plt.close('all')
    error_df=pd.DataFrame({"Iteration":ite, "Loss": los, "Cylinder": cylinder, "Adaptive":a, "Adaptive Weight":aw_list})
...
def postProcess3(filename, xmin, xmax, ymin, ymax, field_ref, field_MIXED, field_diff, s=2, alpha=0.5, marker='o'):
    ...
    if diff == 'absolute':
        ...
        cf = ax[0, 2].scatter(x_diff, y_diff, c=u_diff, vmin=-0.15, vmax=0.15, alpha=alpha, 
                              edgecolors='none', cmap='coolwarm', marker=marker, s=int(s))
...
if __name__ == "__main__":
    ...
    INLET = [-5.0, -5.0] + [0.0, 10.0] * lhs(2, inlet_pts)
    y_INLET = INLET[:,1:2]
    ...
    print(XY_c.shape)
    print(CYLD.shape)
    print(OUTLET_n.shape)

    # Visualize the collocation points
    fig, ax = plt.subplots()
    ax.set_aspect('equal')
    plt.scatter(XY_c_plot[:,0:1], XY_c_plot[:,1:2], marker='o', alpha=0.1 ,color='blue', label="Collocation Points")
    plt.scatter(XY_c_refine[:,0:1], XY_c_refine[:,1:2], marker='o', alpha=0.1 ,color='red', label="Refined Collocation Points")
    #plt.scatter(XY_c_refine[:,0:1], XY_c_refine[:,1:2], marker="+", alpha =0.3, color="yellow")
    ...
    with tf.device('/device:GPU:0'):
        ...
        if debug == 0:
            if optimizer==1:
                ...
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, adaptive_a, aw_list = model.train(iter=6500, learning_rate=0.0005)
            else:
                loss_CYLD, loss_CYLD_neumann, loss_internal, loss_OUTLET_dirichlet, \
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, adaptive_a, aw_list = model.train(iter=10000, learning_rate=0.0005)
        
        elif debug == 1:
            
                loss_CYLD, loss_CYLD_neumann, loss_internal, loss_OUTLET_dirichlet, \
                loss_OUTLET_neumann, loss_f, loss, iterate_count, loss_list, adaptive_a, aw_list = model.train(iter=60, learning_rate=0.0005)
        ...
        x_PINN = np.linspace(-5.0, 9.33, 251)   #251 equally spaced points means 250 grid spaces
        x_PINN1 = np.linspace(-5.0, 9.33, 251)
        y_PINN = np.linspace(-5.0, 5.0, 101)    #100 grid spaces for y axis
        y_PINN1 = np.linspace(-5.0, 5.0, 101)        
        ...
        mse(ite=iterate_count, los=loss_list, cylinder=loss_CYLD, a= adaptive_a, aw_list=aw_list)      
        if diff == 'absolute':
    ...
    postProcess3('uvp_PINN_vs_OF_diff_SBC', xmin=-2.0, xmax=2,ymin=-2.0, ymax=2.0, field_ref=field_ref_int,field_MIXED=field_MIXED, field_diff=field_diff, s=5, alpha=1)
